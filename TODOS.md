...
- **Type guard hygiene: `filePath` input in PDF handlers** (Low) — `pdfInfoHandler` and `mergeOnlyHandler` cast `r.filePath as string` without an explicit `typeof r.filePath !== 'string'` check. Both fail safely today (Node throws `TypeError` caught by the outer try-catch), but an explicit guard documents intent and surfaces the error earlier with a clear message. Identified by `/cso` audit 2026-04-27. → v1.7.1 candidate.
